...
struct param_wrapper final: base_param_ref
{
  explicit param_wrapper (param_smob *s)
  {
    m_var_type = s->type;
    if (var_type_uses_string (m_var_type))
...
pascm_print_param_smob (SCM self, SCM port, scm_print_state *pstate)
{
  ...
  value = pascm_param_value (param_wrapper (p_smob), GDBSCM_ARG_NONE, NULL);
		     ...
		     struct cmd_list_element **show_list)
{
  ...
  switch (param_type)
    ...
    case var_enum:
      ...
      param_wrapper (self).set<var_enum> (self->enumeration[0]);
      break;
...
static SCM
pascm_param_value (base_param_ref const &var, int arg_pos, const char *func_name)
{
  ...
  switch (var.type ())
    ...
    case var_filename:
      {
	const std::string str = var.get<var_string,
					var_string_noescape,
    ...
    case var_enum:
      {
	const char *str = var.get<var_enum> ();
	if (str == nullptr)
	  str = "";
  ...
  if (pascm_is_parameter (self))
    {
      ...
      return pascm_param_value (param_wrapper (p_smob), SCM_ARG1, FUNC_NAME);
    }
  else
    {
      SCM except_scm;
